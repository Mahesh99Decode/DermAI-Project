...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (storedName) {
        ...
        if (reportUserName) reportUserName.value = storedName;
    }
...
function analyze() {
  ...
  fetch("https://dermai-project.onrender.com/predict", {
    ...
    .then(data => {
      ...
      if (data.disease) {
        displayReport(data.disease, data.confidence);
        addToHistory(data.disease, data.confidence);
      } else if (data.error) {
        alert("Analysis failed: " + data.error);
      } else {
        alert("Analysis failed: Unknown response from server.");
      }
    })
    .catch(err => {
      ...
      analyzeBtn.innerHTML = originalText;
      analyzeBtn.disabled = false;
      alert("Analysis failed. Please ensure the backend ML model is running.");
    });
}
...
function displayReport(prediction, confidence) {
  ...
  // Pre-fill Save Report form
  const diseaseInput = document.getElementById('reportDiseaseInput');
  const confidenceInput = document.getElementById('reportConfidenceInput');
  if (diseaseInput) diseaseInput.value = prediction;
  if (confidenceInput) confidenceInput.value = confidence;

  const cancerRiskBadge = document.getElementById('cancerRiskBadge');
  if (cancerRiskBadge) cancerRiskBadge.classList.add('hidden');

  // Generic natural description for all results
  const summary = `The AI model has analyzed your image and detected visual characteristics consistent with ${prediction}.`;
  const prescription = "Please consult with a certified dermatologist for a formal diagnosis and a personalized treatment plan.";
...
function addToHistory(prediction, confidence) {
  ...
  li.innerHTML = `
    ...
    <div class="history-summary">
      ...
      ${(prediction.toLowerCase().includes("skin cancer") || prediction.toLowerCase().includes("skincancer")) ? '<strong style="color:#ef4444;">High Priority Review Recommended</strong>' : 'Routine monitoring suggested.'}
    </div>
  `;
    ...
    .then(data => {
      ...
      if (data && data.length > 0) {
        ...
        data.reverse().forEach(report => {
          const li = document.createElement("li");
          const dateStr = report.created_at ? new Date(report.created_at).toLocaleDateString() : new Date().toLocaleDateString();
          const confidence = report.confidence || 0;
          const prediction = report.Disease_Condition || report.prediction || "Unknown";
...
function handleBooking(event) {
  ...
  const bookingData = {
    user_id: localStorage.getItem("dermAI_userId") || 1, // fallback to 1 if not found
...
function handleSaveReport(event) {
  ...
  const reportData = {
    user_id: localStorage.getItem("dermAI_userId") || 1,
    user_name: document.getElementById("reportUserName").value,
    prediction: document.getElementById("reportDiseaseInput").value,
    confidence: parseFloat(document.getElementById("reportConfidenceInput").value) || 0,
  ...
  .then(res => res.json())
  .then(data => {
    if (data.success || data.message === "Report saved") {
      ...
      setTimeout(() => {
        ...
        submitBtn.innerHTML = originalHtml;
        submitBtn.style.background = "linear-gradient(135deg, #ec4899, #be185d)";
        submitBtn.disabled = false;
      }, 1500);
    } else {
      alert("Failed to save report: " + (data.error || data.message || "Unknown error"));
      submitBtn.innerHTML = originalHtml;
      submitBtn.disabled = false;
    }
  })
